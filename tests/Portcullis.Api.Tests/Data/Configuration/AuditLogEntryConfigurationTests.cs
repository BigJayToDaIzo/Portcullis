...
public class AuditLogEntryConfigurationTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task AuditLogEntry_UserId_HasForeignKey_ToUser()
    {
        await using var context = CreateContext();
        await context.Database.EnsureCreatedAsync(TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task AuditLogEntry_Action_IsStoredAsString()
    {
        await using var context = CreateContext();
        await context.Database.EnsureCreatedAsync(TestContext.Current.CancellationToken);
        ...
        var property = context.Model
            ...
            .FindProperty(nameof(AuditLogEntry.Action))!;
